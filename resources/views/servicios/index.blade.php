...
<td>
<span class="status @switch($s->estado)                                @case('asignado') status-pending @break                                @case('en_proceso') status-active @break                                @case('finalizado') status-success @break                                @case('cancelado') status-danger @break                            @endswitch">
...
<a href="{{ route('servicios.show', $s) }}" class="btn btn-sm btn-secondary">Ver</a>                                @if (auth()->user()->isEmpleado() && $s->estado !== 'finalizado' && $s->estado !== 'cancelado')                                <a href="{{ route('servicios.edit', $s) }}" class="btn btn-sm btn-primary">Editar</a>                                @endif                            </div>
</td>
</tr>                    @empty                    <tr>
<td colspan="9" class="text-center text-gray-500 py-8">No hay servicios registrados.</td>
</tr>                    @endforelse                </tbody>
</table>
</div>
