...
<div class="max-w-2xl mx-auto">
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('empleados.update', $empleado) }}" class="form-grid">
                @csrf @method('PUT')
                ...
                <div class="form-group col-span-full">
                    ...
                    <select id="role" name="role" required onchange="cambiarRol(this.value)">
                        ...
                        <option value="admin" @selected(old('role', $empleado->usuario?->role) === 'admin')>Administrador</option>
                ...
                <div class="form-group">
                    <label for="password">Nueva contraseña <span class="text-gray-400 text-xs">(dejar vacío para mantener)</span></label>
                ...
                <div id="seccionLaboral" style="display:none">
                    ...
                    <div class="form-group">
                        ...
                        <select id="oficina_id" name="oficina_id">
                            ...
                            <option value="{{ $oficina->id }}" @selected(old('oficina_id', $empleado->oficina_id) == $oficina->id)>{{ $oficina->nombre }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input id="puesto" name="puesto" type="text" value="{{ old('puesto', $empleado->puesto) }}">
                        <x-input-error :messages="$errors->get('puesto')" />
                    </div>
                </div>

                {{-- ═══════ DATOS DEL OPERADOR (solo rol operador) ═══════ --}}
                @php $op = $empleado->operador; @endphp
                <div id="seccionOperador" style="display:none">
                    ...
                    <div class="form-group">
                        ...
                        <select id="licencia_tipo" name="licencia_tipo">
                            <option value="">Seleccionar...</option>
                            <option value="A" @selected(old('licencia_tipo', $op?->licencia_tipo) === 'A')>A - Auto particular</option>
                            <option value="B" @selected(old('licencia_tipo', $op?->licencia_tipo) === 'B')>B - Autobús / Camión</option>
                            <option value="C" @selected(old('licencia_tipo', $op?->licencia_tipo) === 'C')>C - Transporte de carga</option>
                            <option value="D" @selected(old('licencia_tipo', $op?->licencia_tipo) === 'D')>D - Servicio público</option>
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input id="licencia_año_vencimiento" name="licencia_año_vencimiento" type="date" value="{{ old('licencia_año_vencimiento', $op?->licencia_año_vencimiento?->format('Y-m-d')) }}">
                    ...
                    <div class="form-group">
                        ...
                        <input id="licencia_vencimiento_federal" name="licencia_vencimiento_federal" type="date" value="{{ old('licencia_vencimiento_federal', $op?->licencia_vencimiento_federal?->format('Y-m-d')) }}">
                        <x-input-error :messages="$errors->get('licencia_vencimiento_federal')" />
                    </div>
                </div>

                {{-- ═══════ DATOS DEL COTIZADOR (solo rol cotizador) ═══════ --}}
                @php $cot = $empleado->cotizador; @endphp
                <div id="seccionCotizador" style="display:none">
                    ...
                    <div class="form-group col-span-full">
                        ...
                        <input id="zona_cobertura" name="zona_cobertura" type="text" value="{{ old('zona_cobertura', $cot?->zona_cobertura) }}" placeholder="Ej: Zona Centro, Estado de México, Nacional...">
