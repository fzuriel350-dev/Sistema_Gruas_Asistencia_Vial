...
<body class="font-sans antialiased bg-[#f0f2f5] text-[#1a1a2e]">
...
</div>            {{-- Nav --}}            <nav class="flex-1 px-2.5 py-3 overflow-y-auto">                @php $isEmpleado = auth()->user()->isEmpleado(); $isAdmin = auth()->user()->isAdmin(); @endphp                <div class="mb-0.5">
...
</div>                @if ($isEmpleado)                <div class="mb-0.5">
...
<div class="mb-0.5">
<div class="text-[10px] uppercase tracking-widest text-white/30 px-3 pt-2.5 pb-1.5 font-semibold">Gestión</div>
...
<span>Reportes</span>
</a>                    @if ($isAdmin)                    <a href="{{ route('configuracion.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-150 {{ request()->routeIs('configuracion.*') ? 'text-[#FFD500] nav-active' : 'text-white/55 hover:text-white/85 hover:bg-white/5' }}">
<svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.066 2.573c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.573 1.066c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.066-2.573c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
</svg>
<span>Configuración</span>
</a>                    @endif                </div>                @else                <div class="mb-0.5">
<div class="text-[10px] uppercase tracking-widest text-white/30 px-3 pt-2.5 pb-1.5 font-semibold">Cotizaciones</div>
...
<a href="{{ route('profile.edit') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-white/40 hover:text-white/70 transition-all duration-150">
<svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
</svg>
<span>Mi Perfil</span>
...
</aside>        {{-- Main Content --}}        <div class="flex-1 flex flex-col min-w-0">            {{-- Topbar --}}            <header class="bg-white px-5 lg:px-7 py-3.5 flex items-center gap-5 border-b border-gray-100 shadow-sm sticky top-0 z-20">                {{-- Mobile toggle --}}                <button class="lg:hidden p-2 -ml-2 rounded-lg hover:bg-gray-100" @@click="sidebarOpen = !sidebarOpen">
...
<div class="p-5 lg:p-7">                    {{ $slot ?? '' }}                    @yield('content')                </div>
</main>
</div>
