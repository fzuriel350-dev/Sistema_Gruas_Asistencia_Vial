...
<div class="form-group" x-data="{tipos: [], cargando: false}" @cargar-tipos.window="cargando = true; fetch('/convenios/aseguradora/' + $event.detail.aseguradoraId + '/tipos').then(r => r.json()).then(data => { tipos = data; cargando = false; document.getElementById('tipo_servicio_id').dispatchEvent(new Event('cambio-tipos')); })">
...
<option :value="t.id" x-text="t.nombre" :selected="t.id == '{{ old('tipo_servicio_id') }}'"></option>
</template>
</select>
